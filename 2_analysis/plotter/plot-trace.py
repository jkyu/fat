...
''' Plots composite figure for a single FMS simulation after
dihedrals and BLAs have already been computed. '''
...
def plot(ic, en_data, angle_data, bla_data):
    ...
    # p2.plot(time, d1112, linestyle='-', color='orchid', label='C10-C11=C12-C13')
    # p2.plot(time, d1314, linestyle='-', color='darkorange', label='C12-C13=C14-C15')
    # p2.plot(time, d15nz, linestyle='-', color='slateblue', label='C14-C15=NZ-CE')
    ...
    for key in spawn_tbf_keys:
        ...
        p3.plot(time, bla, linestyle='--', color='black', label=label1)
        p1.scatter(time[spawn_ind], en_gap[spawn_ind], color='red', label=label2)
        ...
        p3.scatter(time[spawn_ind], bla[spawn_ind], color='red')
