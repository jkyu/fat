...
''' Plot dihedral angle evolution in time per IC.
Each TBF is plotted separately, colored by electronic state, so
maybe you can tell how an individual IC evolves. 
This only works for one dihedral angle at a time right now.
I'll figure out how to get this to work with more, but it'll
make for a messy plot so it might not be worth doing anyway. '''
