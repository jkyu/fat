...
def sort_spawns(ics, datadir, fmsdir, nstates, topfile, angles, bonds):
    ...
    for ic in ics:
        ...
        for tbf_key in data.keys():
            ...
            if tbf_id > 1 and state_id==1:
                
                parent_id = tbf['spawn_info']['parent_state']
                # Consider a modification that ignores upfunneling
                ...
                spawns.append(spawn_geom)
                CH_bonds = md.compute_distances(spawn_geom, bonds) * 10.
    ...
    all_spawns = md.join(spawns)
    all_spawns.save_xyz('fff.xyz')
...
fmsdir = '../../../'
angles = [ [2,5,3], [0,4,1] ]
bonds = [ [2,4], [3,4], [0,5], [1,5] ]
sort_spawns(ics, datadir, fmsdir, nstates, topfile, angles, bonds)
